...
def prepare_train_dataset(entity_data_path: str, last_token_only: bool = False):
    bio_data = []
...
def prepare_test_dataset(entity_data_path: str):
    bio_data = []
    ...
    with open(save_path, "w") as f:
        for item in tqdm(test_data):
            ...
            for choice in choices:
                if user_tag.endswith("\n"):
                    if assistant_tag.endswith("\n"):
                        sent = f"{user_tag}{question}\n{assistant_tag}{choice}"
                    else:
                        sent = f"{user_tag}{question} {assistant_tag} {choice}"
                else:
                    sent = f"{user_tag} {question} {assistant_tag} {choice}"
    ...
    for item in tqdm(test_data):
        ...
        for choice in item["choices"]:
            if user_tag.endswith("\n"):
                if assistant_tag.endswith("\n"):
                    sent = (
                        f"{user_tag}Question: {question}\n\n"
                        f"Answer: {choice} (true or false)\n\n"
                        f"I'm in an exam and the above is a true/false question. I'm not sure whether the answer is true or false. Can you help me?\n"
                        f"{assistant_tag}Sure! The given answer is"
                    )
                else:
                    sent = (
                        f"{user_tag}Question: {question}\n\n"
            ...
            score = 0.0
            token_score = model(
                **tokenizer(sent, return_tensors="pt").to(model.device)
            ).logits[0, -1]
            score += token_score[pos_id].item()
            score -= token_score[neg_id].item()

            scores.append(score)

        score_max_idx = scores.index(max(scores))
        label_idx = item["labels"].index(1)
        acc.append(1 if score_max_idx == label_idx else 0)
...
if __name__ == "__main__":
    ...
    print(tokenizer("is true").input_ids, tokenizer("is false").input_ids)
    pos_id = int(tokenizer("is true").input_ids[-1])
    neg_id = int(tokenizer("is false").input_ids[-1])

    entity_df_path = prepare_train_dataset(args.entity_data_path, args.last_token_only)
    entity_test_df_path = prepare_test_dataset(args.entity_test_data_path)
