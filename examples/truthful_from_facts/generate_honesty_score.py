...
def ours_honesty_function_dataset(
    ...
    if do_test:
        print(f"Test data: {len(train_data)}")
    else:
...
def prepare_train_dataset(entity_data_path: str, last_token_only: bool = False):
    df = pd.read_csv(entity_data_path)

    entity_df_path = entity_data_path.replace(".csv", "_new.csv")
    ...
    for i, row in df.iterrows():
        ...
        label = row["label"]
        if label == 1:
            pos_sent.append(statement)
        elif label == 0:
            neg_sent.append(statement)
        else:
    ...
    # add pos and neg one by one
    for i in range(min(len(pos_sent), len(neg_sent))):
        df.loc[2 * i] = "Tell me a statement about the world facts.\n" + pos_sent[i], 1
        df.loc[2 * i + 1] = (
            "Tell me a statement about the world facts.\n" + neg_sent[i],
            0,
        )
